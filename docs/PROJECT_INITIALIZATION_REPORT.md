...
| **Grade-driven power + open market = pay-to-win**                       | High     | Certain if unresolved  | Decouple grade from combat power (§7, needs approval)                                           | Resolved: grade never affects duel resolution         |
| **Regulatory exposure** from randomised grading with tradeable outputs  | High     | Medium                 | No paid randomness; grading consumes earned materials only; publish odds; age gating            | Resolved: earned-only, no paid randomness             |
...
| **Scope** — the MVP list (§23) is still a year of work                  | Medium   | High                   | Recommend deferring the shared hub out of MVP (§7, needs approval)                              | Resolved: single-player hub first, MP in Phase 6      |
