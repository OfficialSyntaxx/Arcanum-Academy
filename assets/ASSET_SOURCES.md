...
# Derived runtime assets — `assets/derived/characters/`

Generated by `npm run build:characters` (`tools/scripts/build-characters.mjs`) from the CC0
sources above; never hand-edited. The script strips normal/ORM maps, shrinks the base colour to
a small WebP, decimates the mesh, quantizes attributes and keeps only the animation clips the
client uses. Humans (`peasant-m`, `peasant-f`, `ranger-m`) come from the Quaternius modular
outfits on the 65-joint Universal rig; `animations.glb` is the skeleton plus six action clips
from Universal Animation Library 2; `wolf`, `armabee`, `ghost` and `ghost-skull` are the graded
creature models. Re-run the script and commit the outputs after changing a source or the lists
in the script.
