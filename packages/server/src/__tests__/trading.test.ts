...
async function harness() {
  ...
  await seed(ALICE, [{ definitionId: SHARD, quantity: 10 }]);
...
describe('offering', () => {
  it('moves assets out of the satchel and into escrow', async () => {
    ...
    const offered = await h.trading.offer(opened.value.id, ALICE, [
  ...
  it('clears both confirmations when the terms change', async () => {
    ...
    // not be binding.
    const changed = await h.trading.offer(opened.value.id, ALICE, [
...
describe('cancelling', () => {
  it('returns every escrowed asset to its owner', async () => {
    ...
    await h.trading.offer(opened.value.id, ALICE, [{ definitionId: SHARD, quantity: 5 }]);
